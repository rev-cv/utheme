...
# Получаем название сайта из настроек WP
...
if wp user get "$ADMIN_USER" --allow-root > /dev/null 2>&1; then
    ...
    wp user update "$ADMIN_USER" --user_pass="$ADMIN_PASS" --role=administrator --display_name="$SELECTED_NAME" --first_name="$FIRST_NAME" --last_name="$LAST_NAME" --allow-root
else
    echo "Creating new admin..."
    wp user create "$ADMIN_USER" "$ADMIN_EMAIL" --user_pass="$ADMIN_PASS" --role=administrator --display_name="$SELECTED_NAME" --first_name="$FIRST_NAME" --last_name="$LAST_NAME" --allow-root
fi
...
HOME_ID=$(wp post create --post_type=page --post_title="$HOME_TITLE" --post_name="home" --post_status=publish --page_template="page.php" --post_author="$ADMIN_ID" --porcelain)
...
ARTICLES_ID=$(wp post create --post_type=page --post_title="$ARTICLES_TITLE" --post_name="articles" --post_status=publish --page_template="page-list.php" --post_author="$ADMIN_ID" --porcelain)
...
for i in {1..5}; do
   CHILD_ID=$(wp post create --post_type=page --post_title="cl$i" --post_status=publish --post_parent=$PARENT_ID --page_template="page.php" --post_author="$ADMIN_ID" --porcelain)
...
for PAGE_KEY in "${LEGAL_PAGES[@]}"; do
    ...
    PAGE_ID=$(wp post create --post_type=page --post_title="$PAGE_TITLE" --post_name="$PAGE_SLUG" --post_status=publish --page_template="page.php" --post_author="$ADMIN_ID" --porcelain)
...
SITEMAP_ID=$(wp post create --post_type=page --post_title="$SITEMAP_TITLE" --post_name="sitemap" --post_status=publish --page_template="page-list.php" --post_author="$ADMIN_ID" --porcelain)
...
NEWS_ID=$(wp post create --post_type=page --post_title="$NEWS_TITLE" --post_name="news" --post_status=publish --page_template="page-news.php" --post_author="$ADMIN_ID" --porcelain)
...
#удаляем Articles page из меню
wp menu item delete $(wp menu item list $MAIN_MENU_ID --field=id | grep $ARTICLES_ID)
