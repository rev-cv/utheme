...
def build(spec_dir: Path) -> dict:
    ...
    for subdir in sorted(d for d in spec_dir.iterdir() if d.is_dir()):
        _walk(subdir, None, _Flags(), pages, menu_register, footer_register, slug_errors)
...
def _walk(
    ...
    if index_file:
        ...
        for child in subdirs:
            _walk(child, slug, _Flags(), pages, menu_register, footer_register, slug_errors)

    elif subdirs:
        # Контейнер без index: дети продвигаются наверх, наследуя флаги
        for child in subdirs:
            _walk(child, parent_slug, flags, pages, menu_register, footer_register, slug_errors)
